...
#### 2.2 数据主权与可选离线闭环（Local-First Architecture）

**痛点。** 云端 SaaS 的问题不只是"可能泄露"，而是**你没有选择权**：数据必须上传到服务商才能使用，存放位置、备份副本、留存周期全由对方决定，"删除"通常只是标记删除。**架构上就不存在"数据不出网"这个选项** —— 哪怕你愿意牺牲模型能力来换保密，也换不到。

**机制。** Local-First 的实质是**把数据边界的决定权交回企业**。它分三层，其中两层与模型选型无关：

| 层 | 保障内容 | 成立条件 |
|---|---|---|
| **存储层** | 数据存放在哪 | **与模型选型无关，始终成立** |
| **访问层** | 谁能碰数据 | **与模型选型无关，始终成立** |
| **推理层** | 数据是否出网 | **由企业选型决定** |

- **存储层** —— 工作区（会话、项目、文件、`provenance.jsonl` / `runs.jsonl` 双日志、notebook）默认在 `~/Documents/OpenScience`；SQLite 只是**派生索引**，JSONL 才是真相源，索引损坏可从日志重建。**即使全程使用云端模型，这些数据资产依然完整留在本机** —— 服务商停服、涨价、变更条款，都不影响你手上的研发记录。工作区同时是**本地 git 仓库**，代码层面禁止配置远端与 push。
- **访问层** —— 三条硬约束同样与模型选型无关：Agent **只能访问当前工作区**（越界路径直接拒绝）；执行命令、删除文件、安装依赖、建立远程连接**均需人工批准**，且"**永不发布 off**"；API key 只进**系统钥匙串**，**绝不写入 provenance、日志、崩溃报告、git 或导出项目**（连 provenance 都不许存 —— 否则审计材料本身就成了泄露源）。本地预览网关默认只监听 `127.0.0.1`。
- **推理层** —— 这一层**确实取决于选型，也是本条的诚实边界**：
  - **本地模型**（Ollama / LM Studio / vLLM，走 `127.0.0.1`）→ **出网流量为零**，回环地址豁免代理，是真正的离线闭环。
  - **云端模型** → **你放进对话的内容会出网**。此时出网范围 = 你主动提交的提示词，**而非全量工作区** —— 这与 SaaS Agent"研发数据全量托管在对方服务器"有本质区别，但仍不等于零出网。

**这条优势到底成立在哪。** 准确的表述不是"我们保证数据不泄露"，而是：

> **"数据不出网"在云端 SaaS 架构里做不到，在这里是一个可选项 —— 选择权在企业手上，不在工具手上。**

保密等级要求零出网，就接本地模型；需要更强模型能力且网络环境可信，就接云端模型；也可以分阶段 —— 探索期用云端、配方定型期切本地。**两种都支持，而 SaaS 只给你一种。**

**边界。** 三条必须说清：① 不要宣称"绝对不出网" —— 只要使用云端模型，提示词就会出网，这条优势是"**可选**"而非"**默认**"；② 本地模型在复杂推理与长文档理解上与云端旗舰存在量级差距，安全与能力是 trade-off，不是白拿；③ "离线闭环"指**数据面**，装包、拉模型、检索文献仍需网络。

...
> 本章讲**怎么用**。软件的技术架构见 §5，技能清单见 §6，安装部署见 §7。
...
其余四类研发性质按 §4 的裁剪规则取子集，阶段数依次为 **9 / 7 / 7 / 6**。
...
1. **装环境** —— 按 §7.2 把 12 个技能部署到 Desktop 用户目录，重启桌面端。
2. **建工作区** —— 按 §7.4 初始化项目工作区（复制 `AGENTS.md` 与开发报告模板）。

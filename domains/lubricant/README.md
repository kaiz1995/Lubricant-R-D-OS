...
#### 2.2.1 痛点：通用 Agent 没有给你选择权

云端 SaaS 的问题不只是"可能泄露"，而是**你没有选择权**：企业绝密的基础油组分、添加剂配比、专有成本与客户工况，必须上传到服务商才能使用 —— 存放位置、备份副本、留存周期全由对方决定，"删除"通常只是标记删除。

**架构上就不存在"数据不出网"这个选项** —— 哪怕你愿意牺牲模型能力来换取保密，也换不到。

#### 2.2.2 机制：Local-First 是三层，不是一句"数据在本地"

很多人把 Local-First 讲成一句"数据都在本地"，太粗。实际上它是三层，**其中两层与模型选型无关，一层由企业选型决定**：
...
- **关键：即使全程使用云端模型，这些数据资产依然完整留在本机** —— 服务商停服、涨价、变更条款，都不影响你手上的研发记录
...
##### 第三层：推理层 —— 这才是分水岭，也是本条的诚实边界

| 模型选型 | 出网情况 | 适用场景 |
|---|---|---|
| **本地模型**（Ollama / LM Studio / vLLM 等，走 `127.0.0.1`） | **出网流量为零**，是真正的离线闭环；且回环地址被豁免代理，本地模型流量不经代理、不出机器 | 配方机密等级最高、完全离线环境 |
| **云端商用模型**（OpenAI / Anthropic / 自建网关） | **你放进对话的内容会出网**；出网范围 = 主动提交的提示词，**而非全量工作区** —— 这与 SaaS Agent"研发数据全量托管在对方服务器"有本质区别，但仍不等于零出网。通道可控（代理支持 `system` / `custom` / `none` 三档，可指向企业自建内网网关） | 需要更强模型能力、且网络环境可信 |
...
正因如此，企业真正需要的是**能自己决定数据边界**的能力。而云端 SaaS 架构天生给不了这个能力 —— 数据必须上传，你只能选择"相信"对方。

Local-First 给的是**选择权**：

> **"数据不出网"在云端 SaaS 架构里做不到，在这里是一个可选项 —— 选择权在企业手上，不在工具手上。**

保密等级要求零出网，就接本地模型（此时是真正的物理隔离）；需要更强模型能力且网络环境可信，就接云端模型；也可以分阶段 —— 探索期用云端、配方定型期切本地。**两种都支持，而 SaaS 只给你一种。**

#### 2.2.4 表述边界（对外沟通注意）

- **不要说"绝对不出网"** —— 只要使用云端模型，提示词就会出网。这条优势是"**可选**"而非"**默认**"，说成默认会被当场证伪。
- **不要说"不可能泄露"** —— 准确说法是"**可以选择让数据不出网**"。把"可选项"讲成"保证"，是这一条最容易犯的错，也是它过去最容易显得牵强的原因。
...
> **关于"为什么选择基于 Lubricant Science 开发"** —— 四大底座优势（科研数据血统追溯 / 数据主权与可选离线闭环 / LLM 语义与确定性计算解耦 / 开箱即用的专业科研基础设施）的完整说明已前置到 **§2 核心优势**。本节只讲架构如何解耦。

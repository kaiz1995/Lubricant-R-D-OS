...
def final_status(value: dict) -> str:
...
def expected_decision_fields(value: dict) -> dict[str, str]:
    gate_id, status = value.get("gate_id", ""), final_status(value)
    return {
        ...
        "result": f"{gate_id} has deterministic gate disposition {status} from the supplied review conditions and evidence.",
        "decision": status,
        "next_action": "Advance only under the state-machine action authorized by this recorded Gate." if status == "GO" else "Retain the supplied review record and resolve or execute its stated disposition without creating a Freeze or Close artifact here.",
    }

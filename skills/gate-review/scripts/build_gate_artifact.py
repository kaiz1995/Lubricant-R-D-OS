...
def main() -> int:
    ...
    project = json.loads((input_path.parent / data["project_artifact"]).read_text(encoding="utf-8")); experiment = json.loads((input_path.parent / data["experiment_artifact"]).read_text(encoding="utf-8")); request = data["gate"]
    artifact = {"schema_version": "0.1.0", "artifact_type": "gate", "project_id": project["project_id"], "stage": "VERIFIED", **expected_decision_fields(request), "evidence": request["evidence"], "experiment_reference": experiment["experiment_id"], **{key: request[key] for key in ("gate_id", "scope", "satisfied_conditions", "unsatisfied_conditions", "evidence_gaps", "risks")}, "gate_status": final_status(request)}
